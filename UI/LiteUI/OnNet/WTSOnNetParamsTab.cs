...
namespace Klyte.WriteTheSigns.UI
{
    public class WTSOnNetParamsTab
    {
        ...
        private Vector2 m_resultsViewScroll;
        private Vector2 m_scrollFilesFolder;
        ...
        private void DrawImagePicker(OnNetInstanceCacheContainerXml item, Rect areaRect)
        {
            ...
            GUIKlyteCommons.DoInHorizontal(() =>
            {
                GUIKlyteCommons.DoInScroll(ref m_resultsViewScroll, false, true, () =>
                {
                ...
                GUIKlyteCommons.DoInVertical(() =>
                {
                    ...
                    GUILayout.Space(8);
                    GUIKlyteCommons.DoInScroll(ref m_scrollFilesFolder, () => GUILayout.Label(m_currentFolderAtlas?.sprites.Where(x => x.name == m_selectedValue).FirstOrDefault()?.texture, new GUIStyle() { }, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(false)));
                }, GUILayout.Width(areaRect.width / 2), GUILayout.Height(areaRect.height - 80));
            });
            GUIKlyteCommons.DoInHorizontal(() =>
            {
                ...
                {
                    GetBackToList(item, item.GetTextParameter(m_currentEditingParam));
                }
            });
        }

        private void DrawFolderPicker(OnNetInstanceCacheContainerXml item, Rect areaRect)
        {
            bool dirtyType = false;
            if (ulong.TryParse(m_searchPropName?.Split('.')[0] ?? "", out ulong wId2))
            {
                ...
                {
                    GUI.SetNextControlName(f_imageProtocolSelect);
                    var modelType = GUILayout.SelectionGrid(m_isLocal ? 1 : 0, v_protocolsFld, v_protocolsImg.Length);
                    dirtyType = m_isLocal != (modelType == 1);
                    if (dirtyType)
                    {
                        m_isLocal = modelType == 1;
        ...
        private void DrawVariablePicker(OnNetInstanceCacheContainerXml item, Rect areaRect)
        {
            bool dirtyType = false;
            GUIKlyteCommons.DoInHorizontal(() =>
            {
            ...
            GUIKlyteCommons.DoInHorizontal(() =>
            {
                ...
                GUIKlyteCommons.DoInVertical(() =>
                {
                    if (m_isVariable)
                    {
                        GUILayout.Label(GetCurrentParamString(), new GUIStyle(GUI.skin.label) { normal = new GUIStyleState() { textColor = Color.yellow } });
                        GUILayout.Space(8);
                        GUIKlyteCommons.DoInScroll(ref m_scrollFilesFolder, () => GUILayout.Label(m_variableDescription, new GUIStyle(GUI.skin.label) { richText = true }));
                    }
                }, GUILayout.Width(areaRect.width / 2), GUILayout.Height(areaRect.height - 80));
            });
            GUIKlyteCommons.DoInHorizontal(() =>
        ...
        private void GetBackToList(OnNetInstanceCacheContainerXml item, TextParameterWrapper paramVal)
        {

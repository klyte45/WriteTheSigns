...
namespace Klyte.DynamicTextProps.UI
{
    ...
    internal class DTPMileageMarkerTab : UICustomControl
    {
        ...
        public void Awake()
        {
            ...
            AddDropdown(Locale.Get("K45_DTP_FONT_MILEAGE_MARKES"), out m_fontSelect, m_uiHelperHS, new string[0], OnSetFont);
            ...
            actionButton.eventClick += (x, t) => DTPUtils.ReloadFontsOf<BoardGeneratorHighwayMileage>(m_fontSelect);
            ...
            m_loadTextDD = AddLibBox<DTPLibTextMeshMileageMarker, BoardTextDescriptorMileageMarkerXml>(Locale.Get("K45_DTP_PROP_TEXT_LIB_TITLE"), m_pseudoTabTextsContainer,
                                    out _, DoCopyText,
            ...
            AddDropdown(Locale.Get("K45_DTP_TEXT_CONTENT"), out m_dropdownTextContent, groupTexts, AVAILABLE_TEXT_TYPES.Select(x => Locale.Get("K45_DTP_OWN_NAME_CONTENT_MM", x.ToString())).ToArray(), SetTextOwnNameContent);
        ...
        private void OnSetFont(int idx)
        {
            if (idx >= 0)
            {
                BoardGeneratorHighwayMileage.Instance.ChangeFont(idx == 0 ? null : m_fontSelect.items[idx]);
            }
        }

        private void SafeActionInTextBoard(Action<BoardTextDescriptorMileageMarkerXml> toDo)
        {
            if (!m_isLoading)
            {
        ...
        private void LoadTabTextInfo(BoardTextDescriptorMileageMarkerXml descriptor)
        {

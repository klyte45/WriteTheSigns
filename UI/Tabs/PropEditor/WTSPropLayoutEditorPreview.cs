            ...
            if (CurrentInfo != default && !m_viewLocked)
            {
                if ((eventParam.buttons & UIMouseButton.Left) != 0)
                {
                    ...
                    if (CurrentTextDescriptor != null)
                    {
                        ...
                        Vector3 textExt = WTSPropRenderingRules.GetTextMesh(FontServer.instance[EditingInstancePreview.FontName ?? WTSController.DEFAULT_FONT_KEY], CurrentTextDescriptor, 0, 0, 0, m_previewRenderer.GetDefaultInstance(), m_previewRenderer.GetDefaultInstance().Descriptor)?.m_mesh?.bounds.extents ?? default;

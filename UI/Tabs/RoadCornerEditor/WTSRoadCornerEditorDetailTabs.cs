...
namespace Klyte.WriteTheSigns.UI
{
    ...
    internal class WTSRoadCornerEditorDetailTabs : UICustomControl
    {
        ...
        public void Awake()
        {
            ...
            AddTextField(Locale.Get("K45_WTS_ROADCORNER_NAME"), out UITextField name, helperSettings, OnSetName);

            helperSettings.AddSpace(5);

            AddDropdown(Locale.Get("K45_WTS_ROADCORNER_PROPLAYOUT"), out UIDropDown m_propLayoutSelect, helperSettings, new string[0], OnPropLayoutChange);
            AddVector3Field(Locale.Get("K45_WTS_ROADCORNER_POSITION"), out UITextField[] m_position, helperSettings, OnPositionChanged);
            AddVector3Field(Locale.Get("K45_WTS_ROADCORNER_ROTATION"), out UITextField[] m_rotation, helperSettings, OnRotationChanged);
            AddVector3Field(Locale.Get("K45_WTS_ROADCORNER_SCALE"), out UITextField[] m_scale, helperSettings, OnScaleChanged);
            AddLibBox<WTSLibRoadCornerRule, BoardInstanceRoadNodeXml>(helperSettings, out UIButton m_copySettings, OnCopyRule, out UIButton m_pasteSettings, OnPasteRule, out _, null, OnLoadRule, GetRuleSerialized);

            UIHelperExtension helperAllowedTypes = helperSpawning.AddGroupExtended(Locale.Get("K45_WTS_ROADCORNER_ALLOWTITLE"), out UILabel roadTypesTitle, out _);
            AddCheckboxLocale("K45_WTS_ROADCORNER_ALLOW_DIRTROADS", out UICheckBox m_allowDirty, helperAllowedTypes, (x) => ToggleAllow(Level.Level1, x));
            AddCheckboxLocale("K45_WTS_ROADCORNER_ALLOW_ALLEYS", out UICheckBox m_allowAlleys, helperAllowedTypes, (x) => ToggleAllow((Level)5, x));
            AddCheckboxLocale("K45_WTS_ROADCORNER_ALLOW_SMALLROADS", out UICheckBox m_allowSmallRoads, helperAllowedTypes, (x) => ToggleAllow(Level.Level2, x));
            AddCheckboxLocale("K45_WTS_ROADCORNER_ALLOW_MEDIUMROADS", out UICheckBox m_allowMediumRoads, helperAllowedTypes, (x) => ToggleAllow(Level.Level3, x));
            AddCheckboxLocale("K45_WTS_ROADCORNER_ALLOW_LARGEROADS", out UICheckBox m_allowLargeRoads, helperAllowedTypes, (x) => ToggleAllow(Level.Level4, x));
            AddCheckboxLocale("K45_WTS_ROADCORNER_ALLOW_HIGHWAYS", out UICheckBox m_allowHighways, helperAllowedTypes, (x) => ToggleAllow(Level.Level5, x));

            AddSlider(Locale.Get("K45_WTS_ROADCORNER_SPAWN_CHANCE"), out UISlider m_spawnChance, helperSpawning, OnChangeSpawnChance, 0, 255, 1);
            AddCheckboxLocale("K45_WTS_ROADCORNER_PLACEALSOONDISTRICTBORDER", out UICheckBox m_placeDistrictBorder, helperSpawning, OnChangeSpawnOnDistrictBorder);
            AddCheckboxLocale("K45_WTS_ROADCORNER_SPAWNONSEGMENT", out UICheckBox m_spawnOnSegment, helperSpawning, OnChangeSpawnOnSegment);
            AddCheckboxLocale("K45_WTS_ROADCORNER_ONLYIFOUTBOUNDTRAFFICFROMNODE", out UICheckBox m_onlyIfOutbound, helperSpawning, OnChangeOnlyIfOutboundTrafficFromNode);

            AddCheckboxLocale("K45_WTS_ROADCORNER_USEDISTRICTCOLOR", out UICheckBox m_useDistrictColor, helperAppearence, OnChangeUseDistrictColor);
            AddCheckboxLocale("K45_WTS_ROADCORNER_APPLYABBREVIATIONS_FULLNAME", out UICheckBox m_applyAbbreviations_full, helperAppearence, OnChangeApplyAbbreviationsFullName);
            AddCheckboxLocale("K45_WTS_ROADCORNER_APPLYABBREVIATIONS_SUFFIX", out UICheckBox m_applyAbbreviations_suffix, helperAppearence, OnChangeApplyAbbreviationsSuffix);

            AddCheckboxLocale("K45_WTS_ROADCORNER_DISTRICTSELECTIONASWHITELIST", out UICheckBox m_districtWhiteList, helperDistricts, OnSetDistrictsAsWhitelist);
            AddCheckboxLocale("K45_WTS_ROADCORNER_DISTRICTSELECTIONASBLACKLIST", out UICheckBox m_districtBlackList, helperDistricts, OnSetDistrictsAsBlacklist);
